        ...
        rows:[
          {
            ...
            link:"mylist",
            upload:"resources/upload.txt", //path that gets uploaded information
            datatype:"js",
            multiple:false
          }, 
          {
...
function settings_button_pressed()
  ...
  if( settings_toggle == 'settings_is_closed')
  {
	...
	if (settings_button_has_been_pressed_before == false) // User has never entered Settings 
	{													  // Page; default settings loaded 
		var pumpData = [];
		...
		settings_button_has_been_pressed_before = true;
		dataCenter.pumpData = pumpData;		
	}
	else // Settings button HAS already been pressed before 
	{
		// Here we need to load the pumpData file 
	}
...
//      on: {"onItemClick": function () {alert("item has just been clicked");}},
  ...
  function settings_handler()
  {
	...
	for (var i = 1; i <= numPumps; i++)
	{
		...
		//var singleStage2 = $.map()
		pumpData.push(singleStage);
	}
	dataCenter.pumpData = pumpData;
	//pumpData = $$("settings_window").getItem("pumpData");
	//pumpData = JSON.stringify(pumpData);
	//pumpData = JSON.stringify(eval("(" + pumpData + ")"));
	//pumpData = pumpData2;
